...
function parseDREFromCSV(rows: string[][], filename: string): DREParseResult {
  ...
  for (let i = startRow; i < rows.length; i++) {
    ...
    const normalDesc = normalizeText(descricao);
    if (normalDesc.includes("ESTOQUE INICIAL")) isInsideCMVBlock = true;

    let classification = isInsideCMVBlock 
      ? { grupo: "CMV" as DREGrupo } 
      : classificarLinhaDRE(descricao, currentGrupo);
    
    currentGrupo = classification.grupo as DREGrupo;

    const numericValues = findNumericValuesInRow(row);
    if (numericValues.length === 0) continue;

    const valor = roundTo2Decimals(parseSimpleBrazilianNumber(numericValues[0]));
    const valorAnterior = numericValues.length > 1 ? roundTo2Decimals(parseSimpleBrazilianNumber(numericValues[1])) : null;
    ...
    entries.push({
      ...
      valor_anterior: valorAnterior,
      raw_row: row.map(String),
      isCMV: isInsideCMVBlock
    });

    if (normalDesc.includes("ESTOQUE FINAL")) isInsideCMVBlock = false;
  }
...
function findNumericValuesInRow(row: string[]): string[] {
  return row.filter(cell => {
    const cleaned = String(cell || "").trim();
    return cleaned && /^-?[\d.,]+$/.test(cleaned) && cleaned.length > 0;
  });
}
...
export async function parseFileToArray(file: File): Promise<string[][]> {
  ...
  throw new Errorfunction parseDREFromXLS(rows: XLSRow[], filename: string): DREParseResult {
  debugLog("=== Iniciando parseDREFromXLS (REGRAS CONTÁBEIS) ===");
  debugLog("Total de linhas:", rows?.length || 0);
...
export async function parseFileToArray(file: File): Promise<string[][]> {
  ...
  throw new Er("Formato não suportado. Use CSV, XLS ou XLSX.");
}
