...
export function parseDRE(data: string[][]): DREData {
  ...
  const receitaBruta = Math.abs(findSubtotalValue(rows, ['receita operacional', 'receita bruta', 'vendas de mercadorias', 'prestação de serviços']));
  ...
  const despesasOperacionais = Math.abs(findSubtotalValue(rows, ['despesas trabalhistas', 'despesas gerais', 'despesas operacionais']));
  ...
  const resultadoFinanceiro = findSubtotalValue(rows, ['resultado financeiro', 'receitas financeiras', 'despesas financeiras']);
  
  const lucroLiquido = findSubtotalValue(rows, ['lucro do exercício', 'lucro líquido', 'resultado do exercício', 'lucro do período']);
  
  const lucroOperacional = findSubtotalValue(rows, ['lucro operacional', 'resultado operacional']) || (lucroBruto - totalDespesas);
...
export function parseBalanco(data: string[][]): BalancoData {
  ...
  // Find PL
  const patrimonioLiquido = Math.abs(findSubtotalValue(rows, ['patrimônio líquido', 'patrimonio liquido', 'capital social']));
